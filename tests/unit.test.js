...
describe('Yale to Fale replacement logic', () => {
  ...
  test('should handle text that has no Yale references', () => {
    ...
    // Content should remain the same except for the Yale->Fale replacement
    expect(modifiedHtml).toContain('<title>Test Page</title>');
    expect(modifiedHtml).toContain('<h1>Hello World</h1>');
    // The word "Yale" is being replaced with "Fale" in the text
    expect(modifiedHtml).toContain('no Fale references');
  });
  ...
  test('should handle case-insensitive replacements', () => {
    ...
    $('body *').contents().filter(function() {
      ...
      const text = $(this).text();
      // Use a function to preserve the case pattern
      const newText = text.replace(/YALE/g, 'FALE')
                         .replace(/Yale/g, 'Fale')
                         .replace(/yale/g, 'fale');
      if (text !== newText) {
